...
namespace Core.Utilities.Helpers.FileHelper
{
  ...
  public class FileHelperManager : IFileHelper
  {
    ...
    {
      if (File.Exists(filePath))// Tekrar if kontrolü ile parSametrede gelen adreste öyle bir dosya var mı diye kontrol ediliyor.
      {
        File.Delete(filePath);//Eğer dosya var ise dosya bulunduğu yerden siliniyor.
      }
    ...
    public string Upload(IFormFile file, string root)
    {
      if (file.Length > 0)//file.Length=>Dosya uzunluğunu bayt olarak alır. burada Dosya gönderil mi gönderilmemiş diye test işlemi yapıldı.
      {
        ...
        string guid = GuidHelper.CreateGuid();//Core.Utilities.Helpers.GuidHelper klasürünün içinde ki GuidManager klasörüne gidersem burada satırda ne yaptığımı anlayacağım
